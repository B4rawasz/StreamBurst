...
export function RootToggle({
	...
	const item = selected ? (
		<>
			<div className="size-9 md:size-5 relative">{selected.icon}</div>
			<div>
			...
			<PopoverContent className="flex flex-col gap-1 min-w-(--radix-popover-trigger-width) overflow-hidden p-1">
				{options.map((item) => {
					...
					return (
						...
						>
							<div className="size-9 md:size-5">{item.icon}</div>
							<div>
								<p className="text-sm font-medium">{item.title}</p>
								<p className="text-[13px] text-fd-muted-foreground empty:hidden">{item.description}</p>
							</div>

							<Check className={cn("ms-auto size-3.5 text-fd-primary", !isActive && "invisible")} />
						</Link>
					);
				})}
			</PopoverContent>
